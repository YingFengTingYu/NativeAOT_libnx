...
mkdir -p "$output"
crypto="$native/libSystem.Security.Cryptography.Native.Libnx.a"
if [[ "${LIBNX_USE_OPENSSL:-0}" == 1 ]]; then
    crypto="$native/openssl-pal/libSystem.Security.Cryptography.Native.OpenSsl.a"
fi
{
    ...
    "$DEVKITA64/bin/aarch64-none-elf-nm" -g --defined-only "$crypto" |
